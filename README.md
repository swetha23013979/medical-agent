...
### Reg no:212223040222
## AIM:
TO create an medical agent using react 
## ALGORITHM:
1.Initialize the React application and load the Agentic AI interface.

2.Capture user input from the UI and validate the request.

3.Send the input to the AI agent for analysis.

4.Agent interprets the goal and determines required actions.

5.Agent plans steps (search, reasoning, tool usage).

6.Execute planned actions sequentially or conditionally.

7.Collect and synthesize results from each action.

8.Generate a final response based on reasoning outcomes.

9.Update the React UI with the AI response and status.

10.Handle errors, store context, and wait for next user input.
## PROGRAM:
```
# main.py
